...
Execute: python migrar_dados_v2.py
"""
import sqlite3
import psycopg2
import sys
...
def migrar():
    ...
    # Verificar que tabelas existem no SQLite
    sc.execute("SELECT name FROM sqlite_master WHERE type='table';")
    tabelas_existentes = [row[0] for row in sc.fetchall()]
    ...
    if 'almoxarifado' in tabelas_existentes:
    ...
    if 'movimentacao' in tabelas_existentes:
    ...
    # Migrar requisições (se existir)
    if 'requisicao' in tabelas_existentes:
