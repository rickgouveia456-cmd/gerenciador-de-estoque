...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-4">
  ...
  <div class="d-flex gap-2 flex-wrap">
    ...
    <a href="/almoxarifado/<?= $id ?>/importar" class="btn btn-outline-secondary btn-sm">
      <i class="bi bi-upload me-1"></i>Importar
    </a>
    <?php endif; ?>
    <a href="/almoxarifado/<?= $id ?>/exportar" class="btn btn-outline-success btn-sm">
      <i class="bi bi-file-earmark-excel me-1"></i>Exportar
    </a>
    <?php if ($u['perfil'] === 'admin'): ?>
    <a href="/almoxarifado/<?= $id ?>/editar" class="btn btn-outline-secondary btn-sm">
...
<form method="GET" class="card p-3 mb-3">
  <div class="row g-2 align-items-end">
    <div class="col-md-4">
      <label class="form-label small fw-semibold mb-1">Buscar item</label>
      <input type="text" name="filtro" class="form-control form-control-sm"
             placeholder="Nome ou código..." value="<?= h($filtro) ?>">
    </div>
    <div class="col-md-3">
...
<!-- Valor total e contadores -->
<div class="d-flex flex-wrap gap-3 mb-3">
  <span class="badge bg-info p-2 fs-sm">
    <i class="bi bi-box-seam me-1"></i><?= count($itens) ?> itens
  </span>
  <?php if ($valorTotal > 0): ?>
  <span class="badge bg-success p-2 fs-sm">
    <i class="bi bi-currency-dollar me-1"></i>Valor estimado: <?= fmt_dinheiro($valorTotal) ?>
  </span>
  <?php endif; ?>
</div>
    ...
    <table class="table table-hover mb-0">
      <thead>
        <tr>
          ...
          <?php if (in_array($u['perfil'], ['admin','almoxarife'])): ?>
          <th style="width:10%" class="text-center">Compra</th>
          <?php endif; ?>
      ...
      ?>
        <tr class="<?= $rowCls ?>">
          ...
          <td class="text-center">
            ...
            ?>
            <form method="POST" action="/item/<?= $it['id'] ?>/status_compra" class="d-inline">
              <?= csrf_field() ?>
...
<div class="mt-3 d-flex gap-2">
  ...
  <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#modalTransferir">
    <i class="bi bi-arrow-left-right me-1"></i>Transferir Itens
  </button>
</div>
<?php endif; ?>
...
<div class="modal fade" id="modalTransferir" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form method="POST" action="/admin/transferir_itens" id="formTransferir">
        <?= csrf_field() ?>
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label class="form-label fw-semibold">Itens para transferir</label>
            <div style="max-height:300px;overflow-y:auto;border:1px solid var(--border);border-radius:8px">
              ...
              <div class="d-flex align-items-center gap-2 p-2 border-bottom">
                ...
                <div class="flex-grow-1">
                  ...
                  <span class="text-muted small ms-2"><?= fmt_qtd((float)$it['quantidade']) ?> <?= h($it['unidade']) ?></span>
                </div>
                <div>
                  <input type="number" name="qtd_<?= $it['id'] ?>" step="0.01" min="0.01"
                         max="<?= $it['quantidade'] ?>"
                         placeholder="Qtd" class="form-control form-control-sm" style="width:80px">
                </div>
              </div>
              <?php endforeach; ?>
            </div>
