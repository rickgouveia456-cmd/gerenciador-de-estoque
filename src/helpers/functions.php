<?php
/**
 * Logi-Prime  Funcoes utilitarias globais
 */
...
function cfg(string $chave, string $padrao = ''): string {
    ...
    } catch (Throwable $e) { $cache[$chave] = $padrao; }
    return $cache[$chave];
}
function cfg_set(string $chave, string $valor): void {
    db()->prepare("INSERT INTO configuracao_sistema (chave, valor) VALUES (?, ?) ON DUPLICATE KEY UPDATE valor = VALUES(valor)")->execute([$chave, $valor]);
}
function tempo_relativo(string $data): string {
    if (!$data) return '-';
    $diff = time() - strtotime($data);
    if ($diff < 60) return 'agora';
    if ($diff < 3600) return (int)($diff/60) . ' min atras';
    if ($diff < 86400) return (int)($diff/3600) . 'h atras';
